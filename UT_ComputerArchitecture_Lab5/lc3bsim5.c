...
//microseq (lab5)
int GetIRD2(int *x)          { return(x[IRD2]); }
int GetIRD3(int *x)          { return(x[IRD3]); }
int GetPAGE_FAULT(int *x)    { return(x[PAGE_FAULT]); }
//int GetIR_FETCH(int *x)      { return(x[IR_FETCH]); }
// pc (lab4)
...
int GetGATE_PAPTE(int *x)    { return(x[GATE_PAPTE]); }
// load (lab4)
...
// load (lab5)
int GetLD_IR_FETCH(int *x)   { return(x[LD_IR_FETCH]); }
int GetLD_VA(int *x)         { return(x[LD_VA]); }
// mux (lab5)
int GetIR_FETCHMUX(int *x)   { return(x[IR_FETCHMUX]); }
int GetM_R_BIT(int *x)   { return(x[M_R_BIT]); }
//int GetREFERENCEBIT(int *x)  { return(x[REFERENCEBIT]); }
// reg (lab4)
...
void eval_micro_sequencer() {
    ...
    */
    ...
    // lab5
    int IRD2 = GetIRD2(CURRENT_LATCHES.MICROINSTRUCTION);
    int IRD3 = GetIRD3(CURRENT_LATCHES.MICROINSTRUCTION);
    int PAGE_FAULT = GetPAGE_FAULT(CURRENT_LATCHES.MICROINSTRUCTION);
    int IR_FETCH = CURRENT_LATCHES.IR_FETCH;
    ...
    int PROTECTION = (CURRENT_LATCHES.MDR & 0x0008) >> 3;
    int VALID = (CURRENT_LATCHES.MDR & 0x0004) >> 2;
    ...
    if (IRD == 0)
    {
        if(COND == 1 & CURRENT_LATCHES.READY == 1)
            ...
            next_state = J | 0x1;
        else if(COND == 5 & (PROTECTION == 0) & (MODE == 1) & (TRAP == 0))              // Protection
            next_state = J | 0x10;
...
void eval_bus_drivers() {
    ...
    */    
    ...
    int A = SR1_VECMUX == 1 ? (VECMUX == 0 ? 0x0202:
                               VECMUX == 1 ? 0x0204:
                               VECMUX == 2 ? 0x0206:
                               VECMUX == 3 ? 0x0208:0x020A): SR1_out;
    int B = SR2_R6MUX == 1 ? (INCDEC2MUX == 0 ? 2: -2): (IR5 == 0 ? REGS[SR2] : 
        (((IR & 0x00000010) >> 4) == 0 ? (IR & 0x0000001F) : (IR | 0xFFFFFFE0)));
    int amount4 = (IR & 0x0000000F);
    // For lab 5
    int PTBR = Low16bits(CURRENT_LATCHES.PTBR);
    int VPN = Low16bits(CURRENT_LATCHES.VA) >> 9;
    int OFFSET = Low16bits(CURRENT_LATCHES.VA) & 0x01FF;
    ...
    // PA(PTE)
    PAPTE_output = (VPN << 1) + PTBR;

    // PA
    PA_output = OFFSET + (Low16bits(MDR) & 0x3E00);
...
void drive_bus() {
    ...
    */  
    ...
    // For lab5
    int GATE_PAPTE = GetGATE_PAPTE(CURRENT_LATCHES.MICROINSTRUCTION);
    int GATE_PA = GetGATE_PA(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    else if(GATE_PAPTE == 1)
        BUS = Low16bits(PAPTE_output);
    else if(GATE_PA == 1)
        BUS = Low16bits(PA_output);
...
void latch_datapath_values() {
    ...
    */       
    ...
    int LD_IR_FETCH = GetLD_IR_FETCH(CURRENT_LATCHES.MICROINSTRUCTION);
    int IR_FETCH = CURRENT_LATCHES.IR_FETCH;
    int IR_FETCHMUX = GetIR_FETCHMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int M_R_BIT = GetM_R_BIT(CURRENT_LATCHES.MICROINSTRUCTION);
    //int REFERENCEBIT = GetREFERENCEBIT(CURRENT_LATCHES.MICROINSTRUCTION);
    int IR12 = (CURRENT_LATCHES.IR & 0x00001000) >> 12;
    ...
    if (LD_MDR == 1)
    {
        ...
        if (M_R_BIT == 1)
        {
            if (((CURRENT_LATCHES.MDR & 0x0001) == 0))
                NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR | 0x0001; 
            else
                NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR;
            if ((IR_FETCH == 0) && (SUMODE == 1) && (IR12 == 1))
                NEXT_LATCHES.MDR = NEXT_LATCHES.MDR | 0x0002;
        }
    }
    ...
    // For lab5
    if (LD_IR_FETCH == 1)
        NEXT_LATCHES.IR_FETCH = IR_FETCHMUX;
